...
namespace Write2HMIService.Screens
{
    class Results : Screen
    {
        ...
        public override void generateShortArr()
        {
            ...
            tempTxt = "תוצאות הצבעה:" + results.voteNbrInSess.ToString();
            ...
            //-----------------------------------------------
            //כתיבת שורה שניה בעד נגד נמנע
            tempTxt = "בעד: " + results.resultsFor.ToString();
            tempTxt += "    ";
            tempTxt += "נגד: " + results.resultsAgainst.ToString();
            tempTxt += "    ";
            tempTxt += "נמנע: " + results.resultsAvoid.ToString();
            //טיפול במקרה אנגלית עברית וסוגריים
            tempTxt = Reorder.ReorderStr(tempTxt);
            //מעבר למערך של בייטים
            byteArr = Encoding.Default.GetBytes(tempTxt);
            ...
            for (int j = 0; j < results.MembersFor.Count(); j = j + 3)
            {
                ...
                if (curritem1.Length > maxMemberName) { curritem1 = curritem1.Substring(0, maxMemberName); }
                ...
                if ((j + 1) < results.MembersFor.Count())
                {
                    ...
                    //לחתוך את שם החכ כך שיכנסו עוד בשורה
                    if (curritem2.Length > maxMemberName) { curritem2 = curritem2.Substring(0, maxMemberName); }
                    ...
                    if ((j + 2) < results.MembersFor.Count())
                    {
                        ...
                        //לחתוך את שם החכ כך שיכנסו עוד בשורה
                        if (curritem3.Length > maxMemberName) { curritem3 = curritem3.Substring(0, maxMemberName); }
